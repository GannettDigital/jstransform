...
func (z *TimesPointers) WriteAvroCF(writer io.Writer, writeTime time.Time) error {
	if z == nil {
	...
	avroWriter, err := container.NewWriter(writer, container.Snappy, 1, timespointers.NewTimesPointers().Schema())
	if err != nil {
		return err
	}
...
func (z *TimesPointers) WriteAvroDeletedCF(writer io.Writer, writeTime time.Time) error {
	if z == nil {
		return errors.New("unable to write a nil pointer")
	}
	if writeTime.IsZero() {
...
func (z *TimesPointers) convertToAvro(writeTime time.Time) *timespointers.TimesPointers {
	aTime := generate.AvroTime(writeTime)
	if z == nil {
		return &timespointers.TimesPointers{AvroWriteTime: aTime, AvroDeleted: true}
	}

	AppearanceURLs_recordSlice := func(in []*TimesPointersFactCheckClaimsAppearanceURLs) []*timespointers.AppearanceURLs_record {
		converted := make([]*timespointers.AppearanceURLs_record, len(in))
		for i, z := range in {
			converted[i] = &timespointers.AppearanceURLs_record{
				Original: z.Original,
				Url:      z.Url,
			}
		}
	...
	FactCheckClaims_recordSlice := func(in []*TimesPointersFactCheckClaims) []*timespointers.FactCheckClaims_record {
		converted := make([]*timespointers.FactCheckClaims_record, len(in))
		for i, z := range in {
			converted[i] = &timespointers.FactCheckClaims_record{
				AppearanceURLs: AppearanceURLs_recordSlice(z.AppearanceURLs),
				Author:         &timespointers.UnionNullString{String: z.Author, UnionType: timespointers.UnionNullStringTypeEnumString},
				Claim:          &timespointers.UnionNullString{String: z.Claim, UnionType: timespointers.UnionNullStringTypeEnumString},
				Date:           &timespointers.UnionNullString{String: z.Date, UnionType: timespointers.UnionNullStringTypeEnumString},
				Rating:         &timespointers.UnionNullString{String: z.Rating, UnionType: timespointers.UnionNullStringTypeEnumString},
			}
		}
		return converted
	}

	return &timespointers.TimesPointers{
		AvroWriteTime:   aTime,
		FactCheckClaims: FactCheckClaims_recordSlice(z.FactCheckClaims),
		NonRequiredDate: func() *timespointers.UnionNullLong {
			var s *timespointers.UnionNullLong
			if z.NonRequiredDate != nil {
				s = &timespointers.UnionNullLong{Long: generate.AvroOptionalTime(z.NonRequiredDate), UnionType: timespointers.UnionNullLongTypeEnumLong}
			}
			return s
		}(),
...
// TimesPointersBulkAvroWriter will begin a go routine writing an Avro Container File to the writer and add each item from the
