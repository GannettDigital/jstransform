...
import (
	...
	"github.com/GannettDigital/jstransform/generate/avro_test_data/avro/nested"

	"github.com/actgardner/gogen-avro/v7/container"
)

func TestNested_WriteAvroCF(t *testing.T) {
	z := &Nested{}
	...
	read, err := nested.DeserializeNestedFromSchema(containerReader, string(containerReader.AvroContainerSchema()))
	if err != nil {
...
func TestNested_WriteAvroDeletedCF(t *testing.T) {
	z := &Nested{} // In real usage at minimum an ID field should be populated
...
func ExampleNestedBulkAvroWriter() {
	input := []*Nested{{}, {}, {}}
	inputChan := make(chan *Nested)

	devnull, _ := os.Open("/dev/null")
	defer devnull.Close()

	errChan := NestedBulkAvroWriter(devnull, time.Now(), inputChan)
