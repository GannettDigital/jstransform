...
// BuildStructs is a backward-compatibility wrapper for BuildStructsRenameFields.
func BuildStructs(schemaPath string, outputDir string, useMessagePack bool) error {
	return BuildStructsRenameFields(schemaPath, outputDir, useMessagePack, nil, nil)
}

// BuildStructsRename is a backward-compatibility wrapper for BuildStructsRenameFields.
func BuildStructsRename(schemaPath string, outputDir string, useMessagePack bool, nameMap map[string]string) error {
	return BuildStructsRenameFields(schemaPath, outputDir, useMessagePack, nameMap, nil)
}

// BuildStructsRenameFields takes a JSON Schema and generates Golang structs that match the schema.
...
//
// The fieldNameMap is used to provide alternate names for fields in the resulting structs.
...
func BuildStructsRenameFields(schemaPath string, outputDir string, useMessagePack bool, nameMap map[string]string, fieldNameMap map[string]string) error {
	if outputDir == "" {
		wd, err := os.Getwd()
		if err != nil {
	...
	for _, allOfPath := range allOfTypes {
		...
		if err := buildStructFile(schemaPath, allOfPath, name, packageName, nil, outputDir, fieldNameMap); err != nil {
	...
	for _, oneOfPath := range oneOfTypes {
		...
		if err := buildStructFile(schemaPath, oneOfPath, name, packageName, embeds, outputDir, fieldNameMap); err != nil {
			return fmt.Errorf("failed to build struct file for %q: %v", name, err)
		}
	}
