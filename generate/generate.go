...
const (
	disclaimer = "// Code generated by github.com/GannettDigital/jstransform; DO NOT EDIT."
	scalarAny  = "scalar Any"
	msgpSuffix = "_msgp"
	msgpMode   = gen.Encode | gen.Decode | gen.Marshal | gen.Unmarshal | gen.Size | gen.Test
)
...
func BuildStructsWithArgs(args BuildArgs) error {
	...
	if err != nil {
		return fmt.Errorf("failed to discover oneOfTypes: %w", err)
	}

	if len(allOfTypes) == 0 && len(oneOfTypes) == 0 || len(properties) > 0 {
		path, err := filepath.Abs(args.SchemaPath)
		if err != nil {
			return fmt.Errorf("failed to determine absolute path of %q: %w", args.SchemaPath, err)
		}
	...
	if args.GenerateGraphQL {
		...
		if err := buildGraphQLFile(args.SchemaPath, name, packageName, args); err != nil {
			return fmt.Errorf("failed to build GraphQL file for %q: %w", name, err)
		}
	}
	...
	for _, allOfPath := range allOfTypes {
		...
		if err != nil {
			return fmt.Errorf("failed to build struct file for %q: %w", name, err)
		}
		if args.GenerateAvro {
			if err := buildAvro(name, path, args); err != nil {
				return fmt.Errorf("failed to build Avro files for %q: %w", packageName, err)
			}
		}
	}
	...
	if args.GenerateMessagePack {
		if err := buildMessagePackFile(args.OutputDir, msgpMode); err != nil {
			return fmt.Errorf("failed to build MessagePack file for %q: %w", packageName, err)
		}
	}
...
func buildAvro(name, path string, args BuildArgs) error {
	...
	if err != nil {
		return fmt.Errorf("failed to build Avro Schema file: %w", err)
	}
	...
	if err := buildAvroSerializationFunctions(avroSchemaPath, args); err != nil {
		return fmt.Errorf("failed to build Avro serialization code: %w", err)
	}
