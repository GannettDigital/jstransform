...
// WalkFunc processes a single schemaInstance within a JSON schema file returning an error on any problems.
...
type WalkFunc func(path string, value json.RawMessage) error

// Walk runs each instance in the JSON schema through the WalkFunc, keeping track of the JSONPath.
...
// walkInstance will recursively walk JSON Schema Instance calling walkFn for the root and for each property within
// an object and each item in an array.
...
func walkInstance(raw json.RawMessage, path string, walkFn WalkFunc) error {
	...
	var i schemaInstance
	if err := json.Unmarshal(raw, &i); err != nil {
		return fmt.Errorf("failed to unmarshal schemaInstance at path %q: %v", path, err)
	}
